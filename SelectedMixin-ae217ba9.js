...
const SelectedMixin = superclass => // eslint-disable-next-line
class SelectedMixin extends superclass {
  ...
  __getFocusableElements() {
    const focusableElements = Object.entries(this.constructor.config.selectors).find(([, val]) => val.focusTarget)[1];
  ...
  __onKeyDown(event) {
    ...
    const elements = this.__getFocusableElements(); // eslint-disable-next-line
...
export { BatchingElement as B, SelectedMixin as S };
